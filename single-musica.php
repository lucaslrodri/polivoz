                    ...
                    if(!empty($music_list_down_mp3)){echo '<h3>Partituras & documentos</h3>';}
                        echo '<table class="table-condensed"><tbody>';
                            foreach ($music_list_down_pdf as $item){
                                ...
                                echo '<td><a class="btn btn-link" href="'.($item['external']=='no'?get_home_url().'/'.$item['code']:$item['code']).'" target="_blank">';
                                echo 'Download';
                                echo '</a></td>';
                                echo '</tr>';
                            }
                    ...
                    echo '<div class="col-md-'.(empty($music_list_down_pdf)?'12':'6').'">';
                        ...
                        echo '<table class="table-condensed"><tbody>';
                            foreach ($music_list_down_mp3 as $item){
                                ...
                                echo '<td>'.$item['title'].'</td>';
                                echo '<td><a class="btn btn-link" href="'.($item['external']=='no'?get_home_url().'/'.$item['code']:$item['code']).'"'.($item['external']=='no'?' download':'').' target="_blank">';
                                echo 'Download';
                                echo '</a></td>';
                                echo '</tr>';
                            }

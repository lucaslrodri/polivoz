...
function polivoz_gallery_shortcode_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_gallery_shortcode_key', true);
    echo '<input type="text" id="polivoz_gallery_shortcode_field" name="polivoz_gallery_shortcode_field" value="' . esc_attr($value) . '" class="widefat"/>';
...
function polivoz_save_gallery_shortcode_data($post_id) {
    ...
    update_post_meta($post_id, '_gallery_shortcode_key', $my_data);
}
...
function polivoz_music_list_shortcode_callback($post) {
    ...
    echo '<p><input type="text" id="polivoz_music_list_shortcode_field" placeholder="Titulo" name="polivoz_music_list_shortcode_field[title]" value="' . esc_attr($value['title']) . '" class="widefat"/></p>';
...
function polivoz_music_list_iframe_callback($post) {
    ...
    for($i=0; $i<=3;$i++){
        ...
        echo '<div id="iframe-music-container-'.$i.'" class="iframe-music-container'.(($i<$qtd)?'':' hidden').'">';
            ...
            echo '<label class="responsive-iframe"> Resposivo <input type="radio" name=polivoz_music_list_iframe_field['.$i.'][responsive] value="yes" '.(($value[$i]['responsive'])=='yes'?'checked':'').'>Sim </input>';
            echo '<input type="radio" name=polivoz_music_list_iframe_field['.$i.'][responsive] value="no" '.(($value[$i]['responsive']=='no')?'checked':'').'>Não </input>';
            echo '</label></p>';
            echo '<p><input type="text" class="widefat iframe-title" id="polivoz_list_iframe_title_'.$i.'" placeholder="Titulo" name="polivoz_music_list_iframe_field['.$i.'][title]" value="'.esc_attr($value[$i]['title']).'"></p>';
...
function polivoz_music_list_pdfs_callback($post) {
    ...
    for($i=0; $i<=$max_list-1;$i++){
        ...
        echo '<div id="pdfs-music-container-'.$i.'" class="pdfs-music-container'.(($i<$qtd)?'':' hidden').'">';
            ...
            echo '<label class="pdfs-external link-externo"> Link externo <input type="radio" name=polivoz_music_list_pdfs_field['.$i.'][external] class="external-yes" value="yes" '.(($value[$i]['external'])=='yes'?'checked':'').'>Sim </input>';
            echo '<input type="radio" name=polivoz_music_list_pdfs_field['.$i.'][external] class="external-no" value="no" '.(($value[$i]['external']=='no')?'checked':'').'>Não </input>';
            echo '</label>';
            echo '<p><input type="text" class="widefat pdfs-title" id="polivoz_list_pdfs_title_'.$i.'" placeholder="Titulo" name="polivoz_music_list_pdfs_field['.$i.'][title]" value="'.esc_attr($value[$i]['title']).'"></p>';
...
function polivoz_music_list_mp3_callback($post) {
    ...
    for($i=0; $i<=$max_list-1;$i++){
        ...
        echo '<div id="mp3-music-container-'.$i.'" class="mp3-music-container'.(($i<$qtd)?'':' hidden').'">';
            ...
            echo '<label class="mp3-external link-externo"> Link externo <input type="radio" name=polivoz_music_list_mp3_field['.$i.'][external] class="external-yes" value="yes" '.(($value[$i]['external'])=='yes'?'checked':'').'>Sim </input>';
            echo '<input type="radio" name=polivoz_music_list_mp3_field['.$i.'][external] class="external-no" value="no" '.(($value[$i]['external']=='no')?'checked':'').'>Não </input></label>';
            echo '<p><input type="text" class="widefat mp3-title" id="polivoz_list_mp3_title_'.$i.'" placeholder="Titulo" name="polivoz_music_list_mp3_field['.$i.'][title]" value="'.esc_attr($value[$i]['title']).'"></p>';
...
function polivoz_visibility_other_callback($post) {
    ...
    $hidden = get_post_meta($post->ID, '_visibility_hidden_key', true);
    $featured = get_post_meta($post->ID, '_visibility_featured_key', true);
...
function polivoz_save_visibility_other_data($post_id) {
    ...
    update_post_meta($post_id, '_visibility_hidden_key', $hidden);
    $featured = $_POST['polivoz_visibility_field_featured'];
    update_post_meta($post_id, '_visibility_featured_key', $featured);
}
...
function polivoz_nota_musical_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_nota_musical_key', true);
    echo '<p><input type="text" id="polivoz_nota_musical_field" pattern="[a-zA-Z]*||[0]{1}" data-postid="' . $post->ID . '" name="polivoz_nota_musical_field" value="' . esc_attr($value) . '" maxlength="2" size="4"/>';
    echo '<span class="nota-musical">' . calculate_nota_musical($post->ID, esc_attr($value)) . '</span></p>';
...
function polivoz_save_nota_musical_data($post_id) {
    ...
    update_post_meta($post_id, '_nota_musical_key', $my_data);
}
...
function polivoz_visibility_callback($post) {
    ...
    echo '<div id="featured-atributes" '.($featured!='checked'?'class="hidden"':'').'>';
        ...
        echo '<p>Caracteres restantes: <span id="count-character">' . (140 - strlen(esc_attr($data['subtitle']))) . '</span>.</p><p>Subtítulos são curtas descrições com uma única linha com até 140 caracteres.';
...
function polivoz_subtitle_callback($post) {
    ...
    echo '<textarea type="text" rows="2" id="polivoz_subtitle_field" name="polivoz_subtitle_field" maxlength="140" class="widefat">' . esc_attr($value) . '</textarea>';
    echo '<p>Caracteres restantes: <span id="count-character">' . (140 - strlen(esc_attr($value))) . '</span>.</p><p>Subtítulos são curtas descrições com uma única linha com até 140 caracteres.';
    echo '<a href="' . menu_page_url('polivoz-admin-page', false) . '#polivoz-guide-subtitulo" target="_blank""> Aprenda mais sobre subtitulos</a></p>';
}
...
function polivoz_subtitle_alt_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_subtitle_alt_key', true);
    echo '<textarea type="text" rows="2" id="polivoz_subtitle_field" name="polivoz_subtitle_alt_field" maxlength="140" class="widefat">' . esc_attr($value) . '</textarea>';
    echo '<p>Caracteres restantes: <span id="count-character">' . (140 - strlen(esc_attr($value))) . '</span>.</p><p>Subtítulos são curtas descrições com uma única linha com até 140 caracteres.';
    echo '<a href="' . menu_page_url('polivoz-admin-page', false) . '#polivoz-guide-subtitulo" target="_blank""> Aprenda mais sobre subtitulos</a></p>';
}
...
function polivoz_save_subtitle_alt_data($post_id) {
    ...
    update_post_meta($post_id, '_subtitle_alt_key', $my_data);
}
...
function polivoz_home_priority_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_home_priority_key', true);
    if (empty($value)) {
    ...
    echo '<input type="number" min="1" max="9" id="polivoz_home_priority_field" name="polivoz_home_priority_field" value="' . esc_attr($value) . '"/>';
...
function polivoz_save_home_priority_data($post_id) {
    ...
    update_post_meta($post_id, '_home_priority_key', $my_data);
}
...
function polivoz_home_type_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_home_type_key', true);
...
function polivoz_save_home_type_data($post_id) {
    ...
    update_post_meta($post_id, '_home_type_key', $my_data);
}
...
function polivoz_home_columns_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_home_columns_key', true);
    echo '<textarea rows="4" class="widefat" id="polivoz_home_columns_field" name="polivoz_home_columns_field">' . esc_attr($value) . '</textarea>';
    echo '<p>Este campo deve ser escrito em HTML ou PHP. É livre o uso de qualquer função do Wordpress.</p>';
}
...
function polivoz_save_home_columns_data($post_id) {
    ...
    update_post_meta($post_id, '_home_columns_key', $my_data);
}
...
function polivoz_home_form_shortcode_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_home_form_shortcode_key', true);
    echo '<input type="text" id="polivoz_home_form_shortcode_field" name="polivoz_home_form_shortcode_field" value="' . esc_attr($value) . '" class="widefat"/>';
...
function polivoz_home_contact_callback($post) {
    ...
    echo '<ul class="form-no-clear">';
    foreach ($list_values as $key => $name) {
        echo '<li><label>' . $name . '</li><li><input type ="text" id="polivoz_home_contact_field_' . $key . '" name="polivoz_home_contact_field[' . $key . ']" value="' . $value[$key] . '" class="widefat"/></label></li>';
    }
    echo '</ul>';
}
...
function polivoz_home_social_callback($post) {
    ...
    echo '<p>Adicione os dados das mídias sociais que vai aparecer na página inicial na seção de contatos. Os endereços devem ser completos incluindo o <code>http://</code>. Ex: <code>https//www.facebook.com/minha_pagina</code>. Caso queira que um item não apareça deixe em branco.</p>';
    echo '<ul class="form-no-clear">';
    foreach ($list_values as $key => $name) {
        echo '<li><label>' . $name . '</li><li><input type ="text" id="polivoz_home_social_field_' . $key . '" name="' . polivoz_home_social_field . '[' . $key . ']" value="' . $value[$key] . '" class="widefat"/></label></li>';
    }
    echo '</ul>';
}
...
function polivoz_alt_permanlink_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_alt_permanlink_key', true);
    echo '<input type="text" id="polivoz_alt_permanlink_field" name="polivoz_alt_permanlink_field" value="' . esc_attr($value) . '" class="widefat"/>';
...
function polivoz_save_alt_permanlink_data($post_id) {
    ...
    update_post_meta($post_id, '_alt_permanlink_key', $my_data);
}
...
function polivoz_show_menu_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_show_menu_key', true);
    if (empty($value)){
        $value = "yes";
    }
...
function polivoz_save_show_menu_data($post_id) {
    ...
    update_post_meta($post_id, '_show_menu_key', $my_data);
}
...
function polivoz_location_address_callback($post) {
    ...
    foreach ($address_fields as $key => $name) {
        echo '<tr>
                ...
                <td><input type="text" id="polivoz_location_address_field_' . $key . '" name="polivoz_location_address_field[' . $key . ']" value="' . $value[$key] . '"" class="address_field_input ' . $address_fields_class[$key] . '">';
...
function polivoz_location_map_callback($post) {
    ...
    $value = get_post_meta($post->ID, '_location_map_key', true);
    echo '<textarea rows="2" class="widefat" id="polivoz_location_map_field" name="polivoz_location_map_field" required>' . esc_attr($value) . '</textarea>';
    echo '<table><tr>';
...
function polivoz_save_location_map_data($post_id) {
    ...
    update_post_meta($post_id, '_location_map_key', $my_data);
}
...
function polivoz_event_date_callback($post) {
    ...
    $timestamp = get_post_meta($post->ID, '_event_timestamp_key', true);
    ...
    echo '<tr>
            ...
            <input type="text" id="polivoz_event_date_field_from_date" name="polivoz_event_date_field[from_date]" required value="' . $value['from_date'] . '"" class="event_input_date event_input_date_time">
            <label for="polivoz_event_date_field_to_date"> até </label>
            <input type="text" id="polivoz_event_date_field_to_date" name="polivoz_event_date_field[to_date]" value="' . $value['to_date'] . '"" class="event_input_date event_input_date_time">
            </td>
        </tr>';
...
function polivoz_save_event_date_data($post_id) {
    ...
    update_post_meta($post_id, '_event_timestamp_key', $time_stamp);
}
